// Ubicación: src/pages/Pesaje/InspeccionB.jsx
...
// Componente EscenaCamion sin cambios
function EscenaCamion({
  ...
  useEffect(() => {
    if (!nodosVisuales) return
    for (const key in statusTable) {
      const estado = statusTable[key]
      const mesh = nodosVisuales[key]
      if (mesh && mesh.material) {
        let color = null
        if (estado === "verde") color = "green"
        else if (estado === "rojo") color = "red"
        else if (estado === "naranja") color = "orange"
        if (color) {
...
export const InspeccionBUI = ({
  ...
  const { camionActual, checklistB, setChecklistB, comentarios, setComentarios, avanzarEtapa } = useProcesoCamion()
  const { user } = useAuth();
  ...
  const confirmInspectionResult = () => {
    // Bloque unificado para los checklists (Abrazaderas y Seguridad)
    if (["abrazaderas", "seguridad"].includes(currentInspection)) {
      let allChecked = false;
      let infoText = "";

      if (currentInspection === "abrazaderas") {
        allChecked = abrazaderasChecks.colocada && abrazaderasChecks.asegurada && abrazaderasChecks.estado;
        infoText = "Colocada, asegurada y en buen estado.";
      } else if (currentInspection === "seguridad") {
        allChecked = seguridadChecks.cadenas && seguridadChecks.boton && seguridadChecks.confinamiento;
        infoText = "Cadenas, botón y confinamiento OK.";
      }

      if (allChecked) {
        const result = "bien";
        if (onInspectionComplete) {
          onInspectionComplete(currentInspection, result);
        }
        setTemporaryData(prev => [...prev, {
          nombre_parte: currentInspection,
          informacion: infoText,
          estado: result,
        }]);
    ...
    // Lógica que ya tenías para las otras inspecciones (radio buttons)
    if (currentInspection === "tapaSuperior") {
      setChecklistB(prev => ({ ...prev, pipingMetalico: inspectionResult === "bien" }));
    } else if (currentInspection === "mangueraCal") {
      setChecklistB(prev => ({ ...prev, pipingGomaBuenEstado: inspectionResult === "bien" }));
    } else if (currentInspection === "mangueraAire") {
      setChecklistB(prev => ({ ...prev, manguerasBuenEstado: inspectionResult === "bien" }));
    }

    if (onInspectionComplete) {
      onInspectionComplete(currentInspection, inspectionResult);
    }
  ...
  // InspeccionB.jsx -> Dentro de InspeccionBUI, después del useEffect
  ...
  const visibleCameraKeys = Object.keys(partsMapping).filter((key) => {
    return partsMapping[key].grupo === "B"
  })

  const totalParts = visibleCameraKeys.length - 1
  const completedParts = Object.keys(partsMapping)
    .filter(key => key !== 'Camera' && key !== 'Checklist')
    .reduce((count, key) => {
      const part = partsMapping[key];
      const esManguera = part.type === 'mangueraCal' || part.type === 'mangueraAire';
      if (esManguera) {
        const tipo = part.type.replace('manguera', '').toLowerCase();
        if (manguerasAsignadas.some(m => m.tipo_manguera.toLowerCase().includes(tipo))) {
          return count + 1;
        }
      } else {
        if (statusTable[part.dataKey]) {
          return count + 1;
        }
      }
            ...
            if (esManguera) {
              ...
              return (
                <li key={part.key} className="summary-item" onClick={() => handleOpenInspection(part.key)}>
                  <span>{part.name}</span>
                  <span className={`status-pill ${estado || 'pendiente'}`}>
                    {estado ? (estado === 'bien' ? 'OK' : 'Falla') : 'Pendiente'}
                  </span>
                </li>
              );
            }
      ...
      {showInspectionDialog && currentInspection && (
        <>
          ...
          <div className="part-form">
            ...
            <h3>{partsMapping[Object.keys(partsMapping).find(k => partsMapping[k].dataKey === currentInspection)]?.name}</h3>
  ...
  return (
    <>
      ...
      <Suspense fallback={null}>
        <EscenaCamion
          modeloVisualPath="/modelo/Silo2.glb"
          modeloAnimacionPath="/modelo/a05(carga).glb"
