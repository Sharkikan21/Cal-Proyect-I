...
def actualizar(n_intervals):
    ...
    if curvas_reactividad:
        ...
        x = [(t - ultima["timestamp_inicio"]).total_seconds() / 60 for t, _ in ultima["datos"]]
        y = [temp for _, temp in ultima["datos"]]

        fig = go.Figure()
        fig.add_trace(go.Scatter(x=x, y=y, mode='lines+markers', name="Temp Molino"))

        fig.update_layout(
            title=f"🧪 Reactividad de cal: {ultima['tipo']} – {ultima['minutos']} min {ultima['segundos']} s",
        ...
        fig = go.Figure()
        fig.update_layout(
            title="Esperando evento de reactividad...",
            template="plotly_dark"
        )
...
# === Lógica de Reactividad basada en subida de corriente del tornillo ===
    ...
    # 1. Alarma de Relación Agua/Cal
    masa_liquida = agua / cal if not es_cero(cal) else 0
    if cal > 0.01 and (masa_liquida < RELACION_MIN or masa_liquida > RELACION_MAX): # Solo si hay consumo de cal
        if masa_liquida < RELACION_MIN:
            alerta_mensajes_list.append(f"📉 ALERTA: Mezcla demasiado concentrada (Relación Agua/Cal: {masa_liquida:.2f}). Límite: >{RELACION_MIN:.1f}")
        else: # relacion > RELACION_MAX
            alerta_mensajes_list.append(f"📈 ALERTA: Mezcla demasiado diluida (Relación Agua/Cal: {masa_liquida:.2f}). Límite: <{RELACION_MAX:.1f}")
            
    porcentaje_solidos = (cal / agua) * 100 if not es_cero(agua) else 0
    if porcentaje_solidos < 20:
        alerta_mensajes_list.append(f"⚠️ Porcentaje de sólidos bajo ({porcentaje_solidos:.1f}%). Límite mínimo: 20%")
    elif porcentaje_solidos > 25:
        alerta_mensajes_list.append(f"⚠️ Porcentaje de sólidos alto ({porcentaje_solidos:.1f}%). Límite máximo: 25%")
        
    
    total_agua_diluida = agua + flujo_adicional
    porcentaje_solidos_dilucion = (cal / total_agua_diluida) * 100 if not es_cero(total_agua_diluida) else 0
    if porcentaje_solidos_dilucion < 10:
        alerta_mensajes_list.append(f"⚠️ Porcentaje de sólidos en dilución bajo ({porcentaje_solidos_dilucion:.1f}%). Límite mínimo: 10%")
    elif porcentaje_solidos_dilucion > 15:
        alerta_mensajes_list.append(f"⚠️ Porcentaje de sólidos en dilución alto ({porcentaje_solidos_dilucion:.1f}%). Límite máximo: 15%")
        
    ps = porcentaje_solidos / 100
    denominador = ps + 2.3 * (1 - ps)
    densidad = (2.3 * 1000) / denominador if not es_cero(denominador) else 0
    if porcentaje_solidos < 20 or porcentaje_solidos > 25:
        alerta_mensajes_list.append(f"⚠️ Densidad interior fuera de rango (calculada: {densidad:.1f} kg/m³) por desbalance en porcentaje de sólidos ({porcentaje_solidos:.1f}%).")
        
    ps_dilucion = porcentaje_solidos_dilucion / 100
    denominador_dilucion = ps_dilucion + 2.3 * (1 - ps_dilucion)
    densidad_dilucion = (2.3 * 1000) / denominador_dilucion if not es_cero(denominador_dilucion) else 0
    if porcentaje_solidos_dilucion < 10 or porcentaje_solidos_dilucion > 15:
        alerta_mensajes_list.append(f"⚠️ Densidad en tanques fuera de rango (calculada: {densidad_dilucion:.1f} kg/m³) por desbalance en sólidos diluidos ({porcentaje_solidos_dilucion:.1f}%).")


    # 2. Alarma de Temperatura
    if temp > TEMP_MAX_CRITICA:
        alerta_mensajes_list.append(f"🔥 ALERTA CRÍTICA: Temperatura Muy Alta ({temp:.1f}°C). Límite: {TEMP_MAX_CRITICA:.1f}°C")
    elif modo == "produciendo" and temp < TEMP_MIN_PRODUCCION:
        alerta_mensajes_list.append(f"❄️ ALERTA: Temperatura Baja en Producción ({temp:.1f}°C). Esperado: >{TEMP_MIN_PRODUCCION:.1f}°C")

    # 3. Alarma de Nivel TK68
    if tk68 > NIVEL_MAX_TANQUE:
        alerta_mensajes_list.append(f"⚠️ ALERTA: Nivel Alto en TK68 ({tk68:.1f}%). Límite: {NIVEL_MAX_TANQUE:.1f}%")
    elif tk68 < NIVEL_MIN_TANQUE:
        alerta_mensajes_list.append(f"⚠️ ALERTA: Nivel Bajo en TK68 ({tk68:.1f}%). Límite: {NIVEL_MIN_TANQUE:.1f}%")

    # 4. Alarma de Nivel TK69
    if tk69 > NIVEL_MAX_TANQUE:
        alerta_mensajes_list.append(f"⚠️ ALERTA: Nivel Alto en TK69 ({tk69:.1f}%). Límite: {NIVEL_MAX_TANQUE:.1f}%")
    elif tk69 < NIVEL_MIN_TANQUE:
        alerta_mensajes_list.append(f"⚠️ ALERTA: Nivel Bajo en TK69 ({tk69:.1f}%). Límite: {NIVEL_MIN_TANQUE:.1f}%")

    # 5. Alarma de Consumo de Cal
    if modo == "produciendo":
        if cal > CONSUMO_CAL_MAX_PROD:
            alerta_mensajes_list.append(f"💠 ALERTA: Consumo de Cal excesivo ({cal:.3f} TON). Límite: {CONSUMO_CAL_MAX_PROD:.3f} TON")
        elif es_cero(cal): # Si está en producción y el consumo es cero (o muy bajo)
             alerta_mensajes_list.append(f"💠 ALERTA: No hay consumo de Cal en modo Producción.")
        elif cal < CONSUMO_CAL_MIN_PROD and not es_cero(cal):
             alerta_mensajes_list.append(f"💠 ALERTA: Consumo de Cal muy bajo en Producción ({cal:.3f} TON). Mínimo: {CONSUMO_CAL_MIN_PROD:.3f} TON")

    if (modo == "lavando" or modo == "inactivo") and not es_cero(cal):
        alerta_mensajes_list.append(f"💠 ALERTA: Consumo de Cal detectado fuera de producción ({cal:.3f} TON). Debería ser 0.")
    
    # 6. Alarma de Flujo de Agua
    if modo == "produciendo" and agua < FLUJO_AGUA_MIN_PROD and not es_cero(agua):
        alerta_mensajes_list.append(f"💧 ALERTA: Flujo de agua bajo en Producción ({agua:.2f} m³/h). Mínimo: {FLUJO_AGUA_MIN_PROD:.2f} m³/h")
    elif modo == "produciendo" and es_cero(agua):
         alerta_mensajes_list.append(f"💧 ALERTA: No hay flujo de agua en modo Producción.")
    
    if modo == "lavando" and agua < FLUJO_AGUA_MIN_LAVADO and not es_cero(agua):
        alerta_mensajes_list.append(f"💧 ALERTA: Flujo de agua bajo en Lavado ({agua:.2f} m³/h). Mínimo: {FLUJO_AGUA_MIN_LAVADO:.2f} m³/h")
    elif modo == "lavando" and es_cero(agua):
        alerta_mensajes_list.append(f"💧 ALERTA: No hay flujo de agua en modo Lavado.")

    if modo == "inactivo" and not es_cero(agua):
        alerta_mensajes_list.append(f"💧 ALERTA: Flujo de agua detectado con sistema Inactivo ({agua:.2f} m³/h). Debería ser 0.")
        
    # 7. Nivel cámara de separación
    if nivel_sep < 5:
        alerta_mensajes_list.append(f"🧪 ALERTA: Nivel muy bajo en cámara de separación ({nivel_sep:.1f}%). Límite: >5%")
    elif nivel_sep > 95:
        alerta_mensajes_list.append(f"🧪 ALERTA: Nivel muy alto en cámara de separación ({nivel_sep:.1f}%). Límite: <95%")

    # 8. Nivel caja de descarga
    if nivel_pumpbox < 40:
        alerta_mensajes_list.append(f"📤 ALERTA: Nivel bajo en caja de descarga ({nivel_pumpbox:.1f}%). Mínimo: 40%")
    elif nivel_pumpbox > 95:
        alerta_mensajes_list.append(f"📤 ALERTA: Nivel crítico en caja de descarga ({nivel_pumpbox:.1f}%). Máximo: 95%")

    modo = determinar_modo_actual(cal, agua, rotary_val, screw_val)
    

    # Si entra a lavado y no estaba antes
    if modo == "lavando" and (modo_anterior != "lavando"):
        tiempo_inicio_lavado = timestamp_fila
        contador_lavados += 1
        print(f"[{tiempo_inicio_lavado.strftime('%Y-%m-%d %H:%M:%S')}] 💧 Lavado n°{contador_lavados} INICIADO")

    # Si estaba lavando y ahora ya no está (producción o inactivo)
    elif modo_anterior == "lavando" and modo != "lavando" and tiempo_inicio_lavado is not None:
        tiempo_fin = timestamp_fila
        duracion = tiempo_fin - tiempo_inicio_lavado
        minutos = int(duracion.total_seconds() // 60)
        segundos = int(duracion.total_seconds() % 60)
        print(f"[{tiempo_fin.strftime('%Y-%m-%d %H:%M:%S')}] ✅ Lavado n°{contador_lavados} FINALIZADO – Duración: {minutos} min {segundos} s")
        tiempo_inicio_lavado = None

    if modo != modo_anterior:
        timestamp_actual = timestamp_fila.strftime("%Y-%m-%d %H:%M:%S")
        print(f"[{timestamp_actual}] 🔄 Cambio de estado: El sistema está ahora en modo: {modo.upper()}")
        modo_anterior = modo

    setpoints_para_evaluacion = {
        "2270-TAHH-11801": {"valor": 75.0},  # Ejemplo: SP para la alarma TAHH es 75°C (el SP base)
        "2270-TAH-11801":  {"valor": 70.0},  # Ejemplo: SP para TAH
        "2270-TAL-11801":  {"valor": 70.0}
    }
    
    alertas_del_json = [] # Inicializar por si 'sensores_json' no está disponible
    if 'sensores_json' in globals() and sensores_json: # Verificar que sensores_json esté cargado
        alertas_del_json = evaluar_sensores_json(
            fila_datos=fila,
            fila_index=fila_actual,
            mapa_columnas=mapa_col_idx,
            setpoints_dict=setpoints_para_evaluacion,
            config_json_sensores=sensores_json_filtrados
        )

    else:
        print("Advertencia: 'sensores_json' no está disponible globalmente o está vacío.")


    # 3. Integra los resultados de las alertas del JSON:
    if alertas_del_json:
        alerta_mensajes_list.extend(alertas_del_json)
    # --- Fin de chequeo de alarmas ---
    alerta_disparada_local = bool(alerta_mensajes_list)
